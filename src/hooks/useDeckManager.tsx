...
export function DeckProvider({ children }: { children: ReactNode }) {
    ...
    if (stored) {
      try {
        ...
        if (Array.isArray(parsedDecks) && parsedDecks.length > 0) {
          ...
          const defaultDeck = { id: crypto.randomUUID(), name: 'New Deck', cards: [] };
  ...
  const deleteDeck = (id: string) => {
    setDecks(prev => {
      ...
      if (filtered.length === 0) {
        const freshDeck = { id: crypto.randomUUID(), name: 'New Deck', cards: [] };
  ...
  return (
    <DeckContext.Provider value={{ decks, activeDeck, setActiveDeckId, updateActiveDeck, createNewDeck, deleteDeck, isMounted }}>
      {children}
