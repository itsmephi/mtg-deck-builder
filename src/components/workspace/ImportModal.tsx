  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-surface-backdrop backdrop-blur-sm">
      <div className="bg-surface-base border border-line-subtle rounded-xl p-6 max-w-md w-full shadow-2xl flex flex-col gap-6">
        <div>
          <h3 className="text-lg font-bold text-content-primary mb-2">Import Decklist</h3>
          <p className="text-sm text-content-tertiary">How would you like to import <strong>{pendingImport.filename}</strong>?</p>
        </div>
        <div className="flex flex-col gap-3">
          <button
            onClick={() => onProcess('current')}
            className="px-4 py-3 bg-surface-raised hover:bg-surface-overlay text-content-primary rounded-lg text-sm font-bold transition-colors text-left"
          >
            Add to Current Deck
            <span className="block text-xs text-content-muted font-normal mt-1">Merges cards into "{activeDeckName}"</span>
          </button>
          <button 
