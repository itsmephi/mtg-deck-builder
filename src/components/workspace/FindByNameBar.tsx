...
export default function FindByNameBar({ showToast, registerFocusFn }: FindByNameBarProps) {
  ...
  const handleInputKeyDown = useCallback(
    ...
    [showPreview, showDropdown, suggestions, focusedIndex, handleCancel, clearAll, handleSelectSuggestion],
  );
  ...
  return (
    <div ref={containerRef} className="relative shrink-0 bg-surface-panel border-b border-line-panel z-[60]">
      {/* Header row — always 52px tall */}
      ...
      {showDropdown && !showPreview && (
        <div
          className="absolute left-0 right-0 top-full bg-surface-raised border border-line-default rounded-b-lg shadow-xl z-30 overflow-y-auto"
          style={{ maxHeight: 320 }}
        >
          {isLoadingAC && (
      ...
      {showPreview && (
        <div className="absolute left-0 right-0 top-full z-[100] bg-surface-panel border border-line-default rounded-b-lg shadow-xl overflow-y-auto" style={{ maxHeight: "80vh" }}>
                    ...
                    <div className="space-y-0.5 text-[11px]">
                      {displayArtist && (
                        <div className="flex gap-2">
                          ...
                          <span className="text-blue-400">{displayArtist}</span>
                        </div>
                      )}
                      <div className="flex gap-2">
                        <span className="text-content-muted w-16 shrink-0">Set</span>
                        <span className="text-blue-400">{selectedPrinting.set.toUpperCase()}</span>
                      </div>
                      <div className="flex gap-2">
              ...
              {printings.length > 1 && (
                <div className="mt-4">
                  <p className="text-[10px] font-semibold text-content-muted uppercase tracking-wider mb-2">
                    Choose art · {printings.length} printings
                  </p>
                  <div
                    className="flex gap-2 overflow-x-auto pt-1 pb-3 px-2"
                    ...
                    {printings.map((p) => {
                      ...
                      return (
                        <button
                          ...
                          style={{ scrollSnapAlign: "start", width: 88, flexShrink: 0 }}
                          className={`rounded-lg overflow-hidden transition-all ${
                            isSelected
                              ? "ring-2 ring-line-focus ring-offset-2 ring-offset-surface-panel"
                              : "opacity-60 hover:opacity-100 hover:ring-1 hover:ring-line-default"
                          }`}
                        >
                          {tileImg ? (
                            <img
                            ...
                            <div
                              className="w-full bg-surface-deep flex items-center justify-center text-[8px] text-content-muted"
                              style={{ aspectRatio: "488/680" }}
                            >
                              {p.set.toUpperCase()}
                            </div>
