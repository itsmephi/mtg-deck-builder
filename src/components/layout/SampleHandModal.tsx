...
export default function SampleHandModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[60] flex items-center justify-center p-2 md:p-10 bg-black/80 backdrop-blur-xl animate-in fade-in duration-300"
      onClick={onClose}
    >
      <div
        className="w-full max-w-[1600px] h-full flex flex-col bg-neutral-900 rounded-2xl border border-neutral-800 shadow-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between border-b border-neutral-800 p-4 md:p-6 bg-neutral-900 z-10">
          ...
          <div className="flex gap-2 md:gap-3">
            <button
              ...
              className="flex items-center gap-2 px-3 md:px-4 py-2 bg-blue-600 border border-blue-500 rounded-lg text-xs md:text-sm font-bold text-white hover:bg-blue-500 disabled:opacity-30 transition-all"
            >
              <PlusCircle className="w-4 h-4" /> Draw
            </button>
            <button
              onClick={shuffleAndDraw}
              className="flex items-center gap-2 px-3 md:px-4 py-2 bg-neutral-800 border border-neutral-700 rounded-lg text-xs md:text-sm font-bold text-white hover:bg-neutral-700 transition-colors"
            >
              <RefreshCw className="w-4 h-4" /> Mulligan
            </button>
            <button
              onClick={onClose}
              className="p-2 bg-neutral-900 border border-neutral-800 rounded-full text-neutral-400 hover:text-white transition-all hover:bg-neutral-800 shadow-sm"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-3 md:gap-5 pb-20 justify-items-center max-w-full mx-auto">
              ...
              {library.length === 0 && (
                <div className="col-span-full mt-10 flex flex-col items-center justify-center p-12 border border-dashed border-neutral-800 rounded-2xl bg-neutral-900/30 w-full max-w-2xl mx-auto">
                  ...
                  <button
                    onClick={shuffleAndDraw}
                    className="mt-4 px-6 py-2 bg-neutral-800 hover:bg-neutral-700 text-white rounded-lg text-xs font-bold uppercase tracking-widest transition-colors"
                  >
                    Reshuffle
                  </button>
                </div>
