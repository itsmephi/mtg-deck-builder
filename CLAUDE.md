...
`v1.34.4` — Find-by-name: resolve the clicked name to the standalone card. Follow-up to 1.34.3 — re-ranking surfaced the standalone *name* at the top, but clicking it could still open the double-faced card. The selection step runs `searchCards(`!"<name>"`)`, and Scryfall's exact-name operator `!"..."` matches a card's **face** names (not just the full `Front // Back` name), so `!"Swords to Plowshares"` returns the standalone *and* any multi-face card with that face; the code then took `results[0]`. New exported `pickCanonical(results, name)` in `src/lib/scryfall.ts` selects the result whose full name exactly matches the requested name (case-insensitive), else `results[0]`. Replaces blind `results[0]` picks in `FindByNameBar.tsx` (`handleSelectSuggestion`, `openExternalCardPreview`, `openWithCard`, price rescue), `page.tsx` (paste/drag `direct-add` + price rescue), and `useDeckImportExport.tsx` (bulk-import price rescue).

Prior: `v1.34.3` — Find-by-name: surface the standalone card. Searching a card name (e.g. `swords to plowshares`) could return a multi-face card — a split/adventure/reversible whose combined `Front // Back` name contains the query as one face — ranked at/above the standalone and visually indistinguishable in the truncated dropdown; worse, the dropdown is capped at the first 8 names *as Scryfall returns them*, so the real standalone could be pushed off the list and become unreachable. New `rankNameSuggestions()` in `FindByNameBar.tsx` re-ranks the raw autocomplete names *before* `slice(0, 8)` — exact full-name match first, then single-face cards, then multi-face (`//`) cards, preserving Scryfall's relevance order within each tier (stable sort); applied to both the debounced fetch and the retry path. Multi-face suggestions (` // ` in the name) now render a small `double-faced` tag so they can't be mistaken for the standalone.
