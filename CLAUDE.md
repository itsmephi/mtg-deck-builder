...
`v1.32.3` — Mobile grid edit-bar legibility & layout. The touch edit bar in `VisualCard` (tap a card's qty badge in grid view) was hard to read and lopsided: a `bg-gradient-to-t … to-transparent` let card art bleed through behind the upper steppers, and the leading owned `✓` shoved the `− owned + / − qty +` numbers off-centre. The bar now uses the desktop overlay's solid `bg-black/85 backdrop-blur-sm`, and is a centred `flex-col`: a symmetric `[− owned +] / [− qty +]` steppers row (matching the desktop overlay) with the owned `✓` toggle on its own line directly below — the bottom-centre spot the desktop owned toggle (the qty badge) occupies. Desktop/pointer behaviour unchanged.
